...
class TealiumTests: XCTestCase {
    ...
    func testInit() {
        ...
        XCTAssertTrue(((tealium?.modulesManager) != nil), "Modules Manager did not init.")
    }
    ...
    func testEnableDisable() {
        ...
        guard let tealium = Tealium(config: defaultTealiumConfig) else {
            XCTFail("Tealium test instance failed to start.")
            return
        }
