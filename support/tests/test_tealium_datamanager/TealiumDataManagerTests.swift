...
class TealiumDataManagerTests: XCTestCase {
    ...
    func testGetRandom(){
        ...
        let volatileData = dataManager.getVolatileData()
        guard let random = volatileData[TealiumKey.random] as? String else {
            XCTFail()
            return
        }
        
        do {
            let otherData = try TealiumDataManager(account: self.account, profile: self.profile, environment: self.env)
            
            let otherVolatileData = otherData.getVolatileData()
            guard let otherRandom = otherVolatileData[TealiumKey.random] as? String else {
                XCTFail()
                return
            }
